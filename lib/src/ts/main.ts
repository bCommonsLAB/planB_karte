...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    const map_options: maplibregl.MapOptions = await getMapConfiguration();

    // Überprüfe, ob das Map-Element existiert
    const mapElement = document.getElementById("map");
    if (!mapElement) {
      console.error("Map-Element '#map' nicht gefunden");
      return;
    }
    ...
    if (!filterGroup) {
      console.error("Filter-Group-Element '#filter-group' nicht gefunden");
      return;
    }
...
async function getMapConfiguration(): Promise<maplibregl.MapOptions> {
  ...
  try {
    ...
    if (!response.ok) {
      console.log("Lokaler Kartenstil nicht verfügbar. Verwende MapTiler.");
